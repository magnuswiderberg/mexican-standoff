...
Frontend (`src/web`, React + Vite + TS): `npm run dev` starts Vite with HMR, proxying `/hub` (WebSockets) to the .NET server on port 5068 — run both for frontend work. `npm run build` type-checks and emits into the server's `wwwroot` (gitignored), so `dotnet run` alone serves the whole app; `dotnet publish` runs the SPA build automatically. Selection timer for manual testing: pick it in the home page's game settings before hosting — the SPA sends it with `CreateGame`, so the `Game__SelectionTimerSeconds` config value never applies to a game started from the UI (it is only the fallback when `CreateGame` sends no settings, as in the integration tests). Dev bots (`Game__Bots__Enabled`, on by default in Development, off in production) let the host (phone lobby) or the monitor add auto-playing bot seats to test >2-player games without extra devices.
